...
# print(column_names)
# print(df.head())
##Convert time UTC to CEST
TARGET_TIMEZONE = 'Europe/Warsaw'

print(df.head())
TARGET_TIMEZONE = 'Europe/Warsaw' 

#make sure that is timedate
df['publishTime'] = pd.to_datetime(df['publishTime'])
df['startTime'] = pd.to_datetime(df['startTime'])

#Delete information about timezone
df['publishTime_No_TZ'] = df['publishTime'].dt.tz_localize(None)
df['startTime_No_TZ'] = df['startTime'].dt.tz_localize(None)
#Set timezone UTC
df['publishTime_UTC_Localised'] = df['publishTime_No_TZ'].dt.tz_localize('UTC')
df['startTime_UTC_Localised'] = df['startTime_No_TZ'].dt.tz_localize('UTC')
#Change timezone UTC to CEST
df['publishTime_Local'] = df['publishTime_UTC_Localised'].dt.tz_convert(TARGET_TIMEZONE)
df['startTime_Local'] = df['startTime_UTC_Localised'].dt.tz_convert(TARGET_TIMEZONE)

#df['publishTime_No_TZ'] = df['publishTime'].apply(lambda x: x.replace(tzinfo=None))
#df['publishTime_UTC_Localised'] = df['publishTime_No_TZ'].dt.tz_localize('UTC')
#df['publishTime_Local'] = df['publishTime_UTC_Localised'].dt.tz_convert(TARGET_TIMEZONE)

#df['startTime_UTC_Localised'] = df['startTime'].dt.tz_localize('UTC')
#df['startTime_Local'] = df['startTime_UTC_Localised'].dt.tz_convert(TARGET_TIMEZONE)
